...
const bottomItems: NavItem[] = [
  ...
  { label: "Settings", href: "/settings", icon: <Settings className="w-5 h-5" />, roles: ["admin", "manager"] },
];
...
export function Sidebar({ currentPath, onNavigate }: SidebarProps) {
  ...
  const visibleNav = navItems.filter((item) => !item.roles || item.roles.includes(role as UserRole));
  const visibleBottom = bottomItems.filter((item) => !item.roles || item.roles.includes(role as UserRole));
